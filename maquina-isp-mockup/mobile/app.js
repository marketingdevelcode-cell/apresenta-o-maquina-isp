// ========================================
// MÁQUINA ISP — SIMULADOR WHATSAPP MOBILE
// ========================================
...
function updateStatusTime() {
  el.statusTime.textContent = getCurrentTime();
}
...
function renderChatList() {
  ...
  Object.values(AGENTS).forEach(agent => {
    ...
    item.onclick = () => openChat(agent.id);
    item.innerHTML = `
...
function openChat(id) {
  ...
  el.chatHeaderName.textContent = agent.name;
  el.chatHeaderStatus.textContent = 'online';
  el.chatHeaderStatus.classList.remove('typing');
  ...
  setTimeout(() => el.messageInput.focus(), 300);
